...
class DiagnosticRunner:
    """Core logic for running multi-stage adapter transfer diagnostics."""
    ...
    def execute(cls, args):
        ...
        # Stage 0: Metadata
        report.metadata_gate = cls.run_metadata_gate(args.source_adapter, args.source_base_model)
        if report.metadata_gate.status == "FAIL":
            print("[Gate] FATAL: Metadata check failed.")
            if not getattr(args, 'force', False): return report
        ...
        if args.target_model:
            report.compatibility_gate = cls.run_compatibility_gate(args.source_base_model, args.target_model)
            
        # Stage 4: Feasibility
        if args.target_model:
            report.feasibility_gate = cls.run_feasibility_gate(
